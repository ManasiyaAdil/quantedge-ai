...
class ServerMarketDataManager {
  ...
  private webSseListeners = new Set<(chunk: string) => void>();
  ...
  private handleTick(topic: string, data: ParsedSocketResponse) {
    ...
    for (const listener of this.webSseListeners) {
      try {
        listener(msg);
      } catch {
        // Ignored if subscriber disconnected
      }
    }
  }
  ...
  /**
   * Web Streams SSE handler for modern Fetch / Edge / Serverless runtimes
   */
  public createWebSseResponse(request?: Request): Response {
      ...
      start: (controller) => {
        listener = (chunk: string) => {
          try {
            ...
            if (listener) {
              this.webSseListeners.delete(listener);
            }
          }
        };
        this.webSseListeners.add(listener);

        // Send initial status and snapshots
        const initStatus = `data: ${JSON.stringify({ type: "STATUS", status: this.getStatus() })}\n\n`;
        try {
          controller.enqueue(encoder.encode(initStatus));
          for (const tick of this.lastVerifiedSnapshot.values()) {
            controller.enqueue(encoder.encode(`data: ${JSON.stringify(tick)}\n\n`));
          }
        } catch {
          // Ignored if stream cancelled immediately
        }
      },
      cancel: () => {
        if (listener) {
